  ...
  try {
    ...
    try {
      ...
      if (username !== existingUser?.username) {
        ...
        db.prepare('UPDATE users SET username = ? WHERE username = ?').run(
          username,
      ...
      if (existingSettings) {
        db.prepare(`
          ...
          WHERE id = ?
        `).run(username, siteTitle?.trim() || 'Van Hub', siteIcon?.trim() || '', existingSettings.id)
      } else {
        db.prepare(`
          INSERT INTO settings (username, siteTitle, siteIcon)
          VALUES (?, ?, ?)
        `).run(username, siteTitle?.trim() || 'Van Hub', siteIcon?.trim() || '')
      }
